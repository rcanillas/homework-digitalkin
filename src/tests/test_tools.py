...
def test_execute_conversation_tool(conversation_tool, monkeypatch):
    ...
    parameters = {"llm_model": "gpt-4o-mini", "context": context, "question": question}
    response = conversation_tool.execute(parameters)
...
def test_execute_context_retrieval_tool(context_retrieval_tool, monkeypatch):
    ...
    response = context_retrieval_tool.execute(parameters)
    assert isinstance(response, str)
    assert len(response) > 0
